@testset "optimization" begin
    ...
    @assert norm(LsqFit.minimizer(results) - [0.0, 2.0]) < 0.01
    ...
    @assert norm(LsqFit.minimizer(results) - [1.0, 1.0]) < 0.01
    ...
    let
        ...
        @assert norm(LsqFit.minimizer(results) - [1.0, 2.0]) < 0.05
    end
    ...
    let
        ...
        LsqFit.minimizer(results)
        @test LsqFit.converged(results)
        @test all(LsqFit.minimizer(results) .>= lower)

        upper=[15.0, 9.0, 15.0]
        results = LsqFit.levenberg_marquardt(R4, [5.0, 5.0, 5.0], upper=upper)
        LsqFit.minimizer(results)
        @test LsqFit.converged(results)
        @test all(LsqFit.minimizer(results) .<= upper)
